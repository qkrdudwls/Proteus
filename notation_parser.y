...
%type <node> input preorder_expr inorder_expr postorder_expr unary_expr
...
input:
    ...
    | NOTATION_PREFIX unary_expr NOTATION_NEWLINE { root = $2; if(notation_mode == 1) preorder(root); else if(notation_mode == 2) postorder(root); free_tree(root); }
...
preorder_expr:
    ...
    | NOTATION_DIV preorder_expr preorder_expr { $$ = createNode('/', 1, $2, $3); }
    | NOTATION_EXP preorder_expr preorder_expr { $$ = createNode('^', 1, $2, $3); }
    | NOTATION_MOD preorder_expr preorder_expr { $$ = createNode('%', 1, $2, $3); }
    | NOTATION_DIGIT { $$ = createLeaf($1); }
    ;
